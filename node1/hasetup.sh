...
if [[ $VISUDO_RC -eq 0 ]]; then
    FILES_VALIDATED=$(echo "$VISUDO_OUT" | grep -cE ":.*OK$" || echo 0)
...
# ─────────────────────────────────────────────────────────────────────────────
# VERIFICACAO FINAL — confirma que sudo entrega as regras ao usuario rsync
# Em vez de dumpar dezenas de linhas, mostra apenas o numero de regras ativas.
# Se algo falhar, ai sim mostra a saida completa para diagnostico.
# ─────────────────────────────────────────────────────────────────────────────
section "VERIFICACAO FINAL"
RULES_OUT=$(sudo -l -U rsync 2>&1 || true)
ACTIVE_RULES=$(echo "$RULES_OUT" | grep -cE '\(root\)\s+NOPASSWD' || echo 0)
EXPECTED_RULES=$(grep -cE "^rsync\s+ALL=" "$SUDOERS_FILE")

if [[ "$ACTIVE_RULES" -ge "$EXPECTED_RULES" ]]; then
    ok "sudo confirma ${ACTIVE_RULES} regras NOPASSWD ativas para rsync (esperadas: ${EXPECTED_RULES})"
elif [[ "$ACTIVE_RULES" -gt 0 ]]; then
    warn "sudo entrega ${ACTIVE_RULES}/${EXPECTED_RULES} regras — diferenca pode indicar conflito em outro sudoers.d"
    echo "$RULES_OUT" | sed 's/^/                /'
else
    warn "sudo -l -U rsync nao retornou regras. Saida completa:"
    echo "$RULES_OUT" | sed 's/^/                /'
fi
